...
##Working with partner after phase 1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0, score1):
        if who == 0:
            score = score0
            total0 = score - running_high
            if total0 > last_score:
                print(total0, "point(s)! That's the biggest gain yet for Player 0")
                return announce_highest(who, total0, score)
            else:
                return announce_highest(who, last_score, score)

        elif who == 1:
            score = score1
            total1 = score - running_high
            if total1 > last_score:
                print(total1, "point(s)! That's the biggest gain yet for Player 1")
                return announce_highest(who, total1, score)
            else:
                return announce_highest(who, last_score, score)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    number_rolls = 10    
    highest_avg = 0
    while k <= number_rolls:
        averaged_dice = make_averaged(roll_dice, trials_count)   ##piazza(passing roll_dice into make_averaged)
        avg_curr = averaged_dice(k, dice)
        if avg_curr > highest_avg:
            highest_avg = avg_curr
            highest_roll = k
        k += 1
    return highest_roll    
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    add_points = free_bacon(opponent_score) + score   ##hints by piazza Anonymous Poet(there will be added points to the player score (because of free_bacon).)
    if is_swap(add_points, opponent_score) and opponent_score > add_points:  ####swap_strategy(24, 1, 1, 6) ---1, 24   24>1 return 0
        return 0
    elif is_swap(add_points, opponent_score) and add_points > opponent_score:    ###swap_strategy(13, 18, 10, 6)---- 18, 13    18>13  return 6
        return num_rolls
    elif add_points == opponent_score and free_bacon(opponent_score) <= cutoff:    
        return num_rolls
