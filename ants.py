...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        if place.ant is None:
            ...
            if not isinstance(place.ant, ContainerAnt) and isinstance(self, ContainerAnt) and self.contained_ant is None:
                self.contained_ant = place.ant
                place.ant = self

            elif not isinstance(self, ContainerAnt) and isinstance(place.ant, ContainerAnt) and place.ant.contained_ant is None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not place.bees and place is not beehive: 
            place = place.entrance 
            count += 1
        if self.min_range > count:
            count += 1
            place = place.entrance
        if place == beehive or self.max_range < count:
            return None 
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in range(len(lst)):
            lst[bee].reduce_armor(ref_dam + result) 
        # END Problem 5
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for bee in b_list:
            bee.reduce_armor(self.damage) # reduce all bees in its place
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.truequeen:
            ...
            while location:
                if location.ant:
                    if isinstance(location.ant, ContainerAnt) and location.ant.contained_ant:
                        if location.ant.contained_ant.ant_doubled == False:
                            location.ant.contained_ant.damage *= 2
                            location.ant.contained_ant.ant_doubled = True
                            location = location.exit

                        elif location.ant.ant_doubled == False:
                            location.ant.damage *= 2
                            location.ant.ant_doubled = True
                            location = location.exit
                        ...
                        if location.ant.ant_doubled == False:
                            location.ant.damage *= 2
                            location.ant.ant_doubled = True
                            location = location.exit
                        else:
